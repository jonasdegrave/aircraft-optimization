...
for i in range(0,len(sweep_w)):
    ...
    for m in range(0,len(mach)):
        ...
        CD, CLmax, dragDict = dt.aerodynamics(airplane,
                                              Mach=mach[m],
                                              altitude=11000,
                                              CL=0.5,
                                              W0_guess=40000*9.81,
...
CL_List = [np.linspace(-0.5, 1.5454459846664212), np.linspace(-0.5, 2.1726437504368183), np.linspace(-0.5, 2.5907755942837496)]
CD_List = [np.zeros(len(CL_List[0])), np.zeros(len(CL_List[1])), np.zeros(len(CL_List[2]))]
...
plt.figure(3)
plt.title('$C_{D}$ x $C_{L}$', fontsize='large', )
plt.xlabel('$C_{D}$')
plt.ylabel('$C_{L}$')
...
for j in range(len(CL_List)):
    ...
    for i in CL_List[j]:
            ...
            CD_List[j][count], CLmax, dragDict = dt.aerodynamics(airplane,
                                                  Mach=Mach_List[j],
